/**
 * ВИТК — Система записей для администрации
 *
 * Список: pages/administrations/index.html  → загружает content/administrations/index.json
 * Пост:  pages/administrations/post.html?id=SLUG  → загружает content/administrations/SLUG.{lang}.md
 *
 * КАК ДОБАВИТЬ ЗАПИСЬ:
 * 1. Добавьте запись в content/administrations/index.json
 * 2. Создайте content/administrations/ВАШ-SLUG.kk.md  (казахский текст)
 * 3. Создайте content/administrations/ВАШ-SLUG.ru.md  (русский текст)
 */
...
(function () {
  ...
  const NEWS_INDEX = ROOT + 'content/administration/index.json';
  ...
  function renderCard(post, lang) {
    ...
    return `
      <article class="post-card" data-tag="${tag}">
        ${post.image
          ? `<img class="post-card-img post-card-img-admin" src="${ROOT}${post.image}" alt="${title}" loading="lazy">`
  ...
  try {
    ...
    grid.innerHTML = `<div class="posts-empty"><div class="empty-icon">📰</div><p>${
      getLang() === 'kk' ? 'Әзірге жазбалар жоқ' : 'Записей пока нет'
    }</p></div>`;
    return;
  }
  ...
  function renderPage() {
    ...
    grid.innerHTML = slice.length
      ...
      : `<div class="posts-empty"><div class="empty-icon">📰</div><p>${
          lang === 'kk' ? 'Әзірге жазбалар жоқ' : 'Записей пока нет'
        }</p></div>`;
  ...
  async function renderPost() {
    ...
    if (postMeta) {
      ...
      if (postMeta.image && imgBox) {
        imgBox.innerHTML = `<img class="post-page-img post-card-img-admin" src="${ROOT}${postMeta.image}" alt="${postMeta.title[lang] || postMeta.title.ru}">`;
      }
    }

    // Загрузить Markdown
    const mdUrl = `${ROOT}content/administration/${id}.${lang}.md`;
    try {
  ...
  function init() {
    ...
    } else if (path.includes('administrations')) {
      renderNewsList();
      document.addEventListener('click', e => {
